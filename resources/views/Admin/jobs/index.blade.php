                      ...
                      <form action="{{ route('admin.jobs.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                          ...
                          <div>
                              ...
                              <div class="relative mt-1">
                                  <select name="category" id="category" class="block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md">
                          ...
                          <tbody class="divide-y divide-gray-200">
                              @foreach($jobs as $job)
                                  <tr>
                                      ...
                                      <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                          <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $job->is_approved ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800' }}">
                                              {{ $job->is_approved ? 'Approved' : 'Pending' }}
                                          </span>
                                          <span class="ml-1 px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $job->is_active ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-800' }}">
                                      ...
                                      <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                          <div class="flex space-x-2">
                                              ...
                                              <a href="{{ route('employer.jobs.edit', $job->id) }}" class="text-yellow-600 hover:text-yellow-900">Edit</a>
                                              @if(!$job->is_approved)
                                              ...
                                              @endif
                                              <form method="POST" action="{{ route('employer.jobs.destroy', $job->id) }}" onsubmit="return confirm('Are you sure you want to delete this job listing?');">
                                                  @csrf
                                                  @method('DELETE')
